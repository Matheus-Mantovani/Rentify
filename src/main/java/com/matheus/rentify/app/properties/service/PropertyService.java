...
import com.matheus.rentify.app.history.PropertyFinancialsHistoryRepository;
import com.matheus.rentify.app.history.PropertyValueHistoryRepository;
...
import com.matheus.rentify.app.leases.LeaseRepository;
import com.matheus.rentify.app.properties.PropertyRepository;
...
public class PropertyService {
    ...
    @Autowired
    public PropertyService(PropertyRepository propertyRepository, PropertyValueHistoryRepository valueHistoryRepository, PropertyFinancialsHistoryRepository financialsHistoryRepository, CityRepository cityRepository, LeaseRepository leaseRepository, PropertyMapper propertyMapper) {
    ...
    public PropertyDetailsResponseDTO updateProperty(Long id, PropertyRequestDTO propertyRequestDTO) {
        ...
        updateEntityFromDto(existingProperty, propertyRequestDTO);
    ...
    private void updateEntityFromDto(Property property, PropertyRequestDTO dto) {
        if (!Objects.equals(dto.cityId(), property.getCity().getId())) {
        ...
        property.setNeighborhood(dto.neighborhood());
        property.setPostalCode(dto.postalCode());
        property.setStatus(dto.status());
        property.setCurrentMarketValue(dto.currentMarketValue());
        property.setCondoFee(dto.condoFee());
        property.setPropertyTaxValue(dto.propertyTaxValue());
        property.setRegistrationNumber(dto.registrationNumber());
        property.setNotes(dto.notes());
    }
