...
import { X, Save, User, MapPin, Phone, Search } from 'lucide-react';
import { tenantService } from '../../services/tenantService';
import { locationService } from '../../services/locationService';

export default function TenantForm({ onClose, onSuccess, tenantToEdit }) {
  ...
  useEffect(() => {
    if (tenantToEdit) {
      setIsLoading(true);
      tenantService.getTenantById(tenantToEdit.id)
        .then(details => {
          setFormData({
            ...details,
            cpf: details.cpf || '',
  ...
  {/* City Autocomplete Logic with Debounce */}
  useEffect(() => {
  ...
  const handleSelectCity = (city) => {
    ...
    // Auto-select state if not selected yet
    if (!formData.stateId) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare payload matching TenantRequestDTO
      const payload = {
        fullName: formData.fullName,
        cpf: formData.cpf.replace(/\D/g, ''), // Strip non-digits for API regex \d{11}
        rg: formData.rg,
      ...
      if (tenantToEdit) {
        await tenantService.updateTenant(tenantToEdit.id, payload);
      } else {
      ...
      console.error(err);
      setError('Erro ao salvar inquilino. Verifique os campos obrigatórios.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            ...
            {tenantToEdit ? 'Editar Inquilino' : 'Novo Inquilino'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* Section: Address (Residence) */}
          <div className="space-y-4">
             <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b pb-2">
              <MapPin className="w-4 h-4" /> Endereço Atual
            </h3>
        ...
        <div className="sticky bottom-0 bg-slate-50 border-t px-6 py-4 flex justify-end gap-3 z-10 rounded-b-2xl">
          <button
            onClick={onClose}
            className="px-6 py-2.5 border border-slate-300 text-slate-700 rounded-lg hover:bg-white transition-all font-medium"
            disabled={isLoading}
          >
            Cancelar
          </button>
          ...
          >
            {isLoading ? 'Salvando...' : <><Save className="w-4 h-4" /> Salvar Inquilino</>}
          </button>
        </div>
